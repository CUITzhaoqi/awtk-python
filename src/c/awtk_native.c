...
pyobject_t wrap_system_info_init(pyobject_t self, pyobject_t pyargs) {
  ret_t ret = 0;
...
//  if (!PyArg_ParseTuple(pyargs, "Oiiii" , &parse_voidp, &x, &y, &w, &h)) {
//    PyErr_SetString(PyExc_TypeError, "invalid arguments");
//    return NULL;
//  }
//
...
pyobject_t wrap_assets_manager_add(pyobject_t self, pyobject_t pyargs) {
  ret_t ret = 0;
  assets_manager_t* am = NULL;
  asset_info_t* info = NULL;
  ...
  ret = (ret_t)assets_manager_add(am, info);
  return Py_BuildValue("i", ret);
}
...
{"assets_manager_add", wrap_assets_manager_add, METH_VARARGS, "assets_manager_add"},
